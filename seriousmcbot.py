...
async def twitter(ctx):
    ...
    view.add_item(button)
    await ctx.send(view=view)
    logging.info("Sent Twitter Link")

@bot.command(aliases=['lp'])
async def liquidity(ctx):
    embed = discord.Embed(
        description="**Providing Liquidity on VVS is quick & easy!**\n\nDoing so is heavily beneficial for the project & will reward you with a percentage of the trading fees generated by Serious Coin on VVS.\n\nBefore adding liquidity, it is essential to understand the concept of Impermanent Loss. If you are not already familiar with the concept of Impermanent Loss, check out the article below from Crypto.com explaining it in detail 👇\n> [Learn about Impermanent Loss, a crucial concept in DeFi liquidity pools](https://crypto.com/university/what-is-impermanent-loss#:~:text=Impermanent%20Loss%20occurs%20when%20the,and%20the%20value%20at%20withdrawal)",
        colour=0x219d55
    )
    button = discord.ui.Button(
        style=discord.ButtonStyle.url,
        label="Provide Liquidity on VVS",
        url="https://vvs.finance/add/CRO/0x7E575f50777f5096f323EB063fD80BA447627060/3000?lptype=v2&minPrice=&maxPrice="
    )
    view = discord.ui.View()
    view.add_item(button)
    await ctx.send(embed=embed, view=view)
    logging.info("Sent LP Providing Link")
